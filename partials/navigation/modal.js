...
const QUERIES = ["Bed", "Sofa", "Malmo", "Chair"];
...
class Searchmodal extends React.Component {
	...
	handleSubmit = (e) => {
		...
		let params = {};
		let checkedKey = this.state.query;

		for (const key of Object.keys(Router.query)) {
			if (key !== "slug" && key !== query) {
				params[key] = Router.query[key];
			}
		}

		if (checkedKey.length > 0) params[query] = checkedKey;
		this.props.onClose();
		Router.pushRoute("product.index", { ...params });
	};

	render() {
		return (
			<div>
				<Dialog open={this.props.open} keepMounted onClose={this.props.onClose} aria-labelledby="alert-dialog-slide-title" aria-describedby="alert-dialog-slide-description">
					<div className="modal-search" tabIndex={-1} role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
							...
							<form onSubmit={this.handleSubmit}>
								<div className="form-group ph-width nopadding-bottom">
									...
									<div className="input-group">
										...
										/>
										<a href="javascript:void(0)" className="input-group-append" data-dismiss="modal" onClick={() => this.props.onClose()}>
								...
								{QUERIES.map((i, k) => {
									return (
										<li key={k}>
											...
											>
												<a onClick={this.props.onClose}>{i}</a>
											</Link>
										</li>
									);
								})}
